...
if not ckpt_file:
    # Сначала проверяем наличие локального чекпоинта
    local_ckpt_path = f"ckpts/{model}/{model}/model_240000_inference.safetensors"
    if os.path.exists(local_ckpt_path):
        ckpt_file = local_ckpt_path
        print(f"Using local checkpoint: {ckpt_file}")
    else:
        # Если локального нет, загружаем с Hugging Face
        ckpt_file = str(cached_path(f"hf://SWivid/{repo_name}/{model}/model_{ckpt_step}.{ckpt_type}"))
        print(f"Downloading checkpoint from Hugging Face: {ckpt_file}")
